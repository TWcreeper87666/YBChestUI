import { world, system, EntityTypes, DynamicPropertiesDefinition } from '@minecraft/server';
...
export class ChestUI {
     ...
     */
    constructor(homePage, target, resetWhenLeave) {
        /** 玩家離線時切換至主頁 (預設為 true) */
    ...
    static getPlayerPage(player) {
        ChestUI.check();
        return player.getDynamicProperty('page');
    }
    static check() {
        if (!loaded)
...
function mcSetting() {
    ...
    world.afterEvents.worldInitialize.subscribe((e) => {
        ...
        e.propertyRegistry.registerEntityTypeDynamicProperties(inventory_def, EntityTypes.get('yb:inventory'));
        available = true;
...
function updateChest(chestUI) {
    world.getPlayers(chestUI.target).forEach((p) => {
        const slot = p.selectedSlot;
        const item = p.getComponent('inventory').container.getItem(slot);
        const hold = item?.typeId === 'minecraft:compass';
        if (p.getDynamicProperty('pre_slot') === slot && !hold)
            return;
...
function updateUI(chestUI) {
    dim.getEntities({ type: 'yb:inventory' }).forEach((i) => {
        if (i.getDynamicProperty('idle'))
            return;
        ...
        const container = i.getComponent('inventory').container;
